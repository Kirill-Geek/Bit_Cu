...
startMenu:
    ...
    case '1':
    { // пункт меню краткая справка
        choose = startHelp(screen);
        break;
    }
    case '2':
    { // пункт меню работы одного оператора
        ...
        while (faults == 0)
        {
            ...
            if (isCrush(mavic))
                mvprintw(44, 20, "%s %s ⛔", mavic->name, iconMavic);
    ...
    case '3':
    {                                                                                               // пункт меню работы двух операторов - алгоритм расписан в case '2'
        ...
        while (faults == 0)
        {
            ...
            if (isCrush(autel))
                mvprintw(44, 155, "%s %s ⛔", autel->name, iconAutel);
            mvprintw(44, 0, "%s %d", iconTime, count / 10);
            int ch = getch(); // Считывание нажатой клавиши
            if (ch == 27)
                break;
    ...
    case '4':
    {                                                                                               // режим работы оператора и компьютера - алгоритм расписан в case '2'
        drone *mavic = initDrone(iconMavic, "Mavic", 0, 20, KEY_UP, KEY_DOWN, KEY_LEFT, KEY_RIGHT); // Инициализация дрона1
        drone *autel = initDrone(iconAutel, "Autel", 0, 155, 'w', 's', 'a', 'd');                   // Инициализация дрона2
        object *space = mapSpaces(screen, iconSpace);
        object *plot = mapPlots(screen, iconPlot);
    ...
    case '5':
    {                                                                                               // автоматический режим согласованной работы 4 дронов - алгоритм расписан в case '2'
        drone *mavic = initDrone(iconMavic, "Mavic", 0, 20, KEY_UP, KEY_DOWN, KEY_LEFT, KEY_RIGHT); // Инициализация дрона1
        ...
        while (faults == 0)
        {
            ...
            if (isCrush(copter))
                mvprintw(44, 65, "%s %s ⛔", copter->name, iconCopter);
            mvprintw(0, 110, "%s %s %d", dro->name, iconDro, countDro);
            if (isCrush(dro))
                mvprintw(44, 110, "%s %s ⛔", dro->name, iconDro);
            mvprintw(0, 155, "%s %s %d", autel->name, iconAutel, countAutel);
            if (isCrush(autel))
                mvprintw(44, 155, "%s %s ⛔", autel->name, iconAutel);
            mvprintw(44, 0, "%s %d", iconTime, count / 10);
